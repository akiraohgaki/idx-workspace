...
curl -fsSL "${CONFIGS_BASE_URL}/vscode/settings.json" -o "${HOME}/.codeoss-cloudworkstations/data/Machine/settings.json" --create-dirs
curl -fsSL "${CONFIGS_BASE_URL}/vscode/extensions.json" -o "${WORK_DIR}/.vscode/extensions.json" --create-dirs

echo '' >>"${HOME}/.bashrc"
#echo "source ${ENV_FILE}" >>"${HOME}/.bashrc"
curl -fsSL "${CONFIGS_BASE_URL}/zsh/bashrc-addition" >>"${HOME}/.bashrc"
